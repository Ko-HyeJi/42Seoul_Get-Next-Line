...
/*   Updated: 2022/02/21 11:07:15 by hyko             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (nl_idx == -1)
	{
		pre_len = ft_strlen(backup[fd]);
		backup[fd] = gnl_expand_backup(backup[fd], fd);
		if ((int)ft_strlen(backup[fd]) == pre_len)
			nl_idx = ft_strlen(backup[fd]);
	}
	if (nl_idx != -1)
	{
		return_line = ft_substr(backup[fd], 0, nl_idx + 1);
		backup[fd] = gnl_cut_backup(backup[fd], return_line);
		return (return_line);
	}
